...
- **Sync-on-thread** — `await asyncio.to_thread(process_place, ...)` in `bot.py`. The whole pipeline (Anthropic + gspread) runs in a worker thread, so even a slow request never blocks polling. New Telegram messages keep flowing while one is being processed.
